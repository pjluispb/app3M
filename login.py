...
st.header('Iglesia de Los Hermanos - Venezuela')
        ...
        if ingresaReg:
            ...
            if verifregDB.count==1:
                ...
                if st.session_state['rol']=='Pastor':
                    ...
                    # switch_page('encupast02edit_ver2')
                else:
                    switch_page('enculidAsigleh')
                    #switch_page('testnewitem')
                
            else:
                st.toast('Error en los datos ingresados')
                st.toast('Intente de nuevo')
                #st.error('''
